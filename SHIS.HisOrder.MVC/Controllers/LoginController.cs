...
using static SHIS.HisOrder.MVC.Models.EnumClass;
...
namespace SHIS.HisOrder.MVC.Controllers
{
    public class LoginController : Controller
    {
        ...
        private readonly SHISContext _db;
        ...
        public LoginController(SHISContext db, IConfiguration config)
        {
            _db = db;
        ...
        {
            var check_pwok = _db.ShisUsers.Where(x =>
...
if (check_pwok.Count() == 1)
{
            ...
            else
            {
                ...
                //讀取該使用者可瀏覽的權限
                var get_this_user_can_view_page_auth = _db.ShisAuths.Where(x =>
                                        x.UserIdno == User_ID
                                );
        ...
        private string Login(LoginPost obj)
        {
            ...
            {
                //確認帳號是否存在
                var check_id_existed = _db.ShisUsers.Where(x => x.UserIdno == obj.User_IDNo);
        ...
        private string ResetPasswordFromUserMobileNumberViaForgotPassword(string User_ID, string User_mobile_phone)
        {
            ...
            //2023.02.21 因電話號碼欄位 擴增(虛擬擴增因為超級瑪莉說話不算話是慣性)， 因此調整 初始化重置密碼  改由  「+886國碼」「7區域號碼」「3121101用戶電話號碼」中的  【用戶電話號碼】作為判斷依據
            //確認帳號是否存在
            var check_id_existed = _db.ShisUsers.Where(x => x.UserIdno == User_ID);
            if (check_id_existed.Count() == 0)
            {
            ...
            //確認密碼是否正確
            var check_Nowpwok = _db.ShisUsers.Where(x =>
                                        x.UserIdno == User_ID
                                                &&
            ...
            {//執行密碼變更
                ...
                _db.ShisUsers.Update(update_obj);
                _db.SaveChanges();
            }
            else
            {
        ...
        {
            //確認帳號是否存在
            var check_id_existed = _db.ShisUsers.Where(x => x.UserIdno == get_login_useridno());
            if (check_id_existed.Count() == 0)
            {
            ...
            ViewData["NationPhoneList"] = _db.ShisCoderefs.Where(c => c.RefCodetype == "NationalPhone" && c.RefCasetype == "Y").OrderBy(c => c.RefShowseq).ToList();
        ...
        public IActionResult user_list(string search_string)
        {
            ...
            IEnumerable<ShisUser> objShisUserList = null;
                if (string.IsNullOrWhiteSpace(search_string))
                {
                    objShisUserList = _db.ShisUsers;//.ToList();
                }
                else 
                {
                    objShisUserList = _db.ShisUsers
                    ...
                    TempData["Show_User_Messages"] += "Searching for \""+ search_string + "\" keyword found "+ objShisUserList .Count()+ " records";
                }
            objShisUserList = objShisUserList.OrderBy(o => o.UserIdno);

            //2023.03.09 add show UserCategory
            var get_UserCategory_list = _db.ShisCoderefs.Where(c => c.RefCodetype == UserCategory_Coderefs_RefCodetype);
            ...
            objShisUserList.ToList().ForEach
                (x => 
                    {
            ...
            return View("UserList", objShisUserList);
        }

        [Authorize(Roles = "UserManagement")]//登入後可依據設定的 專案名稱 project_id 作為判斷依據
        public IActionResult auth_setting(string account_id)
        {
            IEnumerable<ShisUser> objShisUserList = _db.ShisUsers.Where(x => x.UserIdno == account_id).OrderByDescending(o => o.CreateTime);//.ToList(); 

            var check_id_existed = objShisUserList;
            if (check_id_existed.Count() == 0)
            {
            ...
            else 
            {
                ...
                showUserAuthsView.user_detail = objShisUserList.FirstOrDefault();

                var get_user_have_auth = _db.ShisAuths.Where(x => x.UserIdno == account_id).OrderByDescending(o => o.CreateTime);
                showUserAuthsView.user_have_auth = get_user_have_auth;

                var get_all_project_list = _db.ShisProjects.OrderBy(o => o.CreateTime);

                List<View_ShisProject> get_all_project_list_to_View = new List<View_ShisProject>();
                foreach (ShisProject one_Project in get_all_project_list)
                {
                    get_all_project_list_to_View.Add(new View_ShisProject(one_Project));
                }

                get_all_project_list_to_View.ToList().ForEach
                (x=> {
                        var check_have_auth = get_user_have_auth.Where(c => c.ProjectId == x.db_ShisProject.ProjectId);
        ...
        private object[] DoAccountBirthDateChange(string? Account,  DateTime? Birthdate)
        {
            ...
            //------------檢查傳入項目，都有填寫完成後--------------
            IEnumerable<ShisUser> objShisUserList = _db.ShisUsers.Where(x => x.UserIdno == Account).OrderByDescending(o => o.CreateTime);//.ToList(); 
        ...
        private string ChangeUserAuthorization(string admin_ID, List<string> Auth_Project_ID_string, string User_ID)
        {
            ...
            //取得使用者目前的權限
            var get_user_have_auth = _db.ShisAuths.Where(x => x.UserIdno == User_ID).OrderByDescending(o => o.CreateTime);
            var get_all_project_list = _db.ShisProjects.OrderBy(o => o.CreateTime);
            //取得全部可設定的權限
            ...
            (x => {
                ...
                {//原本就有
                    ...
                    { //Delete
                        ...
                        //確認是否正確
                        var check_Nowpwok = _db.ShisAuths.Where(x_get_check =>
                        ...
                        {//執行變更
                            ...
                            _db.ShisAuths.Remove(delete_obj);
                            _db.SaveChanges();
                            change_record = true;

                            //write log
                            ShisAuthsLog log_obj = new ShisAuthsLog();
        ...
        public ActionResult DoAddAccount(string Account, string FirstName, string MidName, string LastName, string CountryPhone, string AreaPhone, string MobilePhone, string MobilePhone2,string Category, string Sex, string mail, DateTime Birthdate)
        {
            ...
            if (_db.ShisCoderefs.Where(c => c.RefCode == Category && c.RefCodetype == UserCategory_Coderefs_RefCodetype).Count() == 0) 
            {
                check_input_error_message += "Category must select a valid item."; 
            }
        ...
        private string CreateAccount(string User_ID, string Account_ID, string FirstName, string MidName, string LastName, string MobilePhone, string UserCategory, string Sex_Only_M_or_F, string Email, DateTime Birth_date)
        {
            ...
            //確認帳號是否存在
            var check_id_existed = _db.ShisUsers.Where(x => x.UserIdno == Account_ID);
            if (check_id_existed.Count() != 0)
            {
        ...
        public IActionResult show_account()
        {
            ...
            ViewBag.UserCategory = _db.ShisCoderefs.Where(c => c.RefCode == chose_one_account.UserCategory && c.RefCodetype == UserCategory_Coderefs_RefCodetype).First().RefName;
        ...
        public string get_login_useridno() 
        {
         ...
         //string get_user_id = get_user_auth_cookie.Where(t => t.Type == "User_ID").First().Value;
            ...
            const string Session_Key = "LoginDTO";
            var get_session = HttpContext.Session.GetObject<SHIS.HisOrder.MVC.Areas.HisOrder.Models.LoginDTO>(Session_Key);
            if (get_session == null) 
            {
                return null;
            }
        ...
        private void write_user_log(string User_ID, string event_type, bool isSuccess, string event_error_input = null, string message = null)
        {
            ...
            //write log
            ShisUsersLog log_obj = new ShisUsersLog();
        ...
        private string AccountLoginLockout(string User_ID,int allow_user_login_error_count,int lock_second)//帳戶鎖定機制
        {
            ...
            int query_error_count_before = _db.ShisUsersLogs.Where(x =>
               x.UserIdno == User_ID
               && x.EventType == "doLogin"
    ...
    public class ShowUserAuths
    {
        ...
        public ShisUser user_detail { get; set; }
        
        public IEnumerable<ShisAuth> user_have_auth { get; set; }
    
        public IEnumerable<View_ShisProject> all_project_list { get; set; }
...
{
    public partial class View_ShisProject
    {//感謝子捷提供方法 解決 View 擴增 虛擬欄位的問題
        public ShisProject db_ShisProject;//private ShisProject _ShisProject;

        public View_ShisProject(ShisProject inShisProject)
        {
            db_ShisProject = inShisProject;
        }
