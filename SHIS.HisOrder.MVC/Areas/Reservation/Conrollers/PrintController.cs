...
namespace SHIS.HisOrder.MVC.Areas.Reservation.Conrollers
{
    ...
    public class PrintController : Controller
    {
        ...
        public PartialViewResult PrintReserveSheet(string InHospID, string patientID,string reserveType)
        {
            ...
            ReservePrintData.HospName = _context.ShisCoderefs.Where(c => c.RefCodetype == "HospitalCode").Select(c => c.RefName).FirstOrDefault().ToString();

            var reserveDtos = _context.Registrations.Where(c => c.Inhospid == InHospID);
            var patientDtos = _context.ShisCharts.Where(c => c.ChrHealthId == patientID);
