...
namespace SHIS.HisOrder.MVC.Areas.HisOrder.Controllers
{
    ...
    public class HisOrderController : Controller
    {
        ...
        public HisOrderController(SHISContext context)
        {
            _context = context;
        }
        ...
        private bool CheckPatientVisitBool(DateOnly inRegDate)
        {
            ...
            {
                // pass visit
                var passData = _context.ShisCoderefs.Where(c => c.RefCodetype == "PassClinicVisit").FirstOrDefault();
        ...
        public string CheckPatientVisit(string patientInhospid, string patientPatientid)
        {
            try
            {
                ...
                var clinicCategory = _context.ShisDepartments.Where(d => d.DptCode == regData.RegDepartment).FirstOrDefault().DptCategory;
        ...
        public ActionResult PatientVisit(string patientInhospid, string patientPatientid, string patientVisitStatus, string htmlBody, string htmlBodyActive)
        {
            ...
            else
            {
                ...
                //取得藥品相關設定
                ViewBag.MedFreq = _context.ShisMedfrequencies.ToList();
                ViewBag.MedIndication = _context.ShisMedfrequencyInds.ToList();
                ViewBag.MedPathWay = _context.ShisMedpathways.ToList();
                ViewBag.MedItem = JsonConvert.SerializeObject(_context.ShisMedicines.ToList());

                //取得非藥相關
                if (_context.ShisCoderefs.Where(c => c.RefCodetype == "NonMedLocation").Count() > 0)
                {
                    ViewBag.NonMedLocation = _context.ShisCoderefs.Where(c => c.RefCodetype == "NonMedLocation").OrderBy(d => d.RefShowseq).ToList();
                }
                else
                {
                    ViewBag.NonMedLocation = new List<ShisCoderef>();
                }
        ...
        private void IniatializeSessionClinicDto(DateTime? regDate, string deptCode, string noonNo, string doctorCode, string loginCode, string authdoctorCode = null)
        {
            ...
            else
            {
                ...
                var deptInfo = _context.ShisDepartments.Where(c => c.DptCode == deptCode).FirstOrDefault();
        ...
        public List<PatientDTO> GetPatientList(DateTime inRegDate, string inDeptCode)
        {
            ...
            var ptData = from d in _context.Set<Registration>()
                         join chart in _context.Set<ShisChart>()
            ...
            if (ptData.Any())
            {
                ...
                var clinicCategory = _context.ShisDepartments.Where(c => c.DptCode == inDeptCode).FirstOrDefault().DptCategory;
        ...
        public PatientDTO GetPatientInfo(string inhospid, string inPatientid)
        {
            ...
            else
            {
                ...
                if (ptData.Any())
                {
                    ...
                    list.ForEach(c =>
                    {
                        var clinicCategory = _context.ShisDepartments.Where(d => d.DptCode == c.RegDept).FirstOrDefault().DptCategory;
                        if (clinicCategory == "EMG")
                        {
                            c.canVisit = true;
                        }
