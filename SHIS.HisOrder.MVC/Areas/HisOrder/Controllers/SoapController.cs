...
namespace SHIS.HisOrder.MVC.Areas.HisOrder.Controllers
{
    ...
    public class SoapController : Controller
    {
        ...
        public SoapController(SHISContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public List<ShisCoderef> getMgInfo()
        {
            try
            {
               return _context.ShisCoderefs.Where(c => c.RefCodetype == "MG_INFO").OrderBy(d=>d.RefShowseq).ToList();
            }
            catch
            {
                return null;
