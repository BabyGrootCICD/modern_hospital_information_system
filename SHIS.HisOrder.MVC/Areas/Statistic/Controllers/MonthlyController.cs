...
namespace SHIS.HisOrder.MVC.Areas.Statistic.Controllers
{
    ...
    public class MonthlyController : Controller
    {
        ...
        public MonthlyController(SHISContext context)
        {
            _context = context;
        }
        ...
        public IActionResult Index()
        {
            ...
            List<ShisDepartment> deptData = GetShisDepartments();
            List<ShisDepartment> deptParents = new List<ShisDepartment>();
            deptParents = deptData.Where(c => string.IsNullOrEmpty(c.DptParent)).ToList();

            JArray Datas = new JArray();
            foreach (ShisDepartment deptItems in deptParents)
            {
                List<ShisDepartment> ChildDept = new List<ShisDepartment>();
        ...
        public List<ShisDepartment> GetShisDepartments()
        {
            var deptData = from d in _context.Set<ShisDepartment>()
                           where d.DptStatus == "Y"
                           //where d.DptParent != null
                           //where d.DptParent != ""
                           orderby d.DptCode
                           select new ShisDepartment
                           {
            ...
            {
                return new List<ShisDepartment>();
            }
        }
