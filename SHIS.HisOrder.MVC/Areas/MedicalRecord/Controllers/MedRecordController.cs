...
namespace SHIS.HisOrder.MVC.Areas.MedicalRecord.Controllers
{
    [Area("MedicalRecord")]
    ...
    public class MedRecordController : Controller
    {
        ...
        public MedRecordController(SHISContext context)
        {
            _context = context;
        }
        ...
        {
            var addres = _context.ShisCoderefs.Where(a => a.RefCodetype == "Chart_Area").ToList();
            return Json(addres);
        }
        public IActionResult phonechecker(string? phone)
        {
            var lphone = "+252 063 " + phone;
            var findnumbers = _context.ShisCharts
        ...
        [HttpPost]
        public IActionResult AddAddress(string address,ShisCoderef coderef)
        {
            ...
            if (coderef.RefName != null)
            {
                ...
                var refshowseq = _context.ShisCoderefs.Where(c => c.RefCodetype == "Chart_Area").Max(r => r.RefShowseq);
        ...
        public IActionResult MRCreate()
        {
            ...
            List<ShisCoderef> areaList = new List<ShisCoderef>();
            List<ShisCoderef> relationList = new List<ShisCoderef>();
            List<ShisCoderef> NationPhoneList = new List<ShisCoderef>();

            List<EnumAnonymous> anonymousList = new List<EnumAnonymous>();
            ShisChart chartData = new ShisChart();
            #endregion
            ...
            areaList = _context.ShisCoderefs.Where(c => c.RefCodetype == "Chart_Area" && c.RefCasetype == "Y").OrderBy(c => c.RefShowseq).ToList();
            relationList = _context.ShisCoderefs.Where(c => c.RefCodetype == "Chart_Relationship" && c.RefCasetype == "Y").OrderBy(c => c.RefShowseq).ToList();
            NationPhoneList = _context.ShisCoderefs.Where(c => c.RefCodetype == "NationalPhone" && c.RefCasetype == "Y").OrderBy(c => c.RefShowseq).ToList();
        ...
        public IActionResult MREdit(string PatientID)
        {
            ...
            var chartDtos = _context.ShisCharts.Where(c =>c.ChrHealthId == PatientID);
