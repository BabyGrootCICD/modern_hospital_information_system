using SHIS.HisOrder.MVC.Areas.MedicalRecord.ViewModels;
...
namespace SHIS.HisOrder.MVC.Areas.MedicalRecord.Models
{
    public class MedicalRecordService : IDisposable
    {
        ...
        public MedicalRecordService(SHISContext context)
        {
            _context = context;
        }
            ...
            string modifyId)
        {
            ...
            try
            {
                ...
                if (false == String.IsNullOrWhiteSpace(mRCreateStructure.PATIENTID) &&
                    _context.ShisCharts.Where(c => c.ChrHealthId == mRCreateStructure.PATIENTID).Any())
                {
            ...
            EnumClass.DisplayLanguage language)
        {
            ...
            List<ShisChart> chartList = new List<ShisChart>();
            ...
            try
            {
                chartList = _context.ShisCharts.Where(c => 1 == 1).ToList();
        ...
        public GeneratePhoneReutrnMsg AddNationalPhoneList(string nationalName, string nationalPhoneCode, EnumClass.DisplayLanguage language, string modifyId)
        {
            ...
            List<ShisCoderef> NationalPhoneList = new List<ShisCoderef>();
            EnumClass.ReplyNoCode replyCode = EnumClass.ReplyNoCode.ReplyOK;
            ...
            try
            {
                ShisCoderef newRef = new ShisCoderef()
                {
                ...
                _context.ShisCoderefs.Add(newRef);
            ...
            ref string generatePatientID)
        {
            ...
            ShisChart newChart = new ShisChart();
            #endregion
            ...
            List<ShisCoderef> newPatientRef = new List<ShisCoderef>();
            bool isTestUse = false;
            string HospCode = "";

            HospCode = _context.ShisCoderefs.Where(c => c.RefCodetype == "HospitalCode" && c.RefCasetype == "Y").Any() ? _context.ShisCoderefs.Where(c => c.RefCodetype == "HospitalCode" && c.RefCasetype == "Y").FirstOrDefault().RefCode : "";
            ...
            {
                newPatientRef = _context.ShisCoderefs.Where(c => c.RefCodetype == "TestChartNoSerialNo" && c.RefCasetype == "Y").ToList();
                isTestUse = true;
            }
            else
            {
                newPatientRef = _context.ShisCoderefs.Where(c => c.RefCodetype == "ChartNoSerialNo" && c.RefCasetype == "Y").ToList();
                isTestUse = false;
            }
            ...
            ShisCoderef ptID = newPatientRef.FirstOrDefault();
            ...
            */
            ...
            ShisCoderef upDate = new ShisCoderef();
            ShisCoderef AnonymousNo = new ShisCoderef();
            if (EnumClass.EnumAnonymous.TestingUse == mRCreateStructure.enumAnonymous)
            {
                upDate = _context.ShisCoderefs.Where(c => c.RefCodetype == "TestChartNoSerialNo" && c.RefCasetype == "Y").First();
            }
            else
            {
                upDate = _context.ShisCoderefs.Where(c => c.RefCodetype == "ChartNoSerialNo" && c.RefCasetype == "Y").First();
            }
            ...
            try
            {
                ...
                switch (mRCreateStructure.enumAnonymous)
                {
                    ...
                    case EnumClass.EnumAnonymous.FemaleKid:
                        ...
                        AnonymousNo = _context.ShisCoderefs.Where(c => c.RefCodetype == "Anonymous_No").First();
                ...
                _context.ShisCharts.Add(newChart);

                //2023.04.28 shis_chart_log add by elain.
            ...
            ref EnumClass.ReplyNoCode replyCode)
        {
            ...
            ShisChart updateChart = new ShisChart();
            #endregion
            ...
            var chartDtos = _context.ShisCharts.Where(c => c.ChrHealthId == mRCreateStructure.PATIENTID);
        ...
        /// <param name="inChart">ShisChart</param>
        private void ModifyMedicalReocrdLog(char inMode, ShisChart inChart)
        {
            //1. ShisChart Dto to json
            var jsonString = JsonConvert.SerializeObject(inChart);

            //2. json to ShisChartLog Dto
            var logDto = (ShisChartLog)JsonConvert.DeserializeObject(jsonString, typeof(ShisChartLog));
            ...
            //3. insert
            _context.ShisChartLogs.Add(logDto);
        }
