...
//using SHIS.HisOrder.MVC.Migrations;
...
namespace SHIS.HisOrder.MVC.Areas.Maintenance.Models
{
    public class ClinicScheduleService : IDisposable
    {
        ...
        public ClinicScheduleService(SHISContext context)
        {
            _context = context;
        }
            ...
            DisplayLanguage language)
        {
            ...
            List<ClinicSchedule> clinicList = new List<ClinicSchedule>();
            List<ShisUser> userList = new List<ShisUser>();
            List<ShisDepartment> deptList = new List<ShisDepartment>();
            ...
            deptList = _context.ShisDepartments.Where(c => c.DptCode == dpt).ToList();
            ...
            {
                userList = _context.ShisUsers.Where(c => c.UserIdno == doctor).ToList();
        ...
        public List<ScheduleData> getCanReserveClinicByDate(DateTime date, string reserveType)
        {
            ...
            using (DepartmentService deptService = new DepartmentService(_context))
            {
                ...
                {
                    var shift = "";
                    var ShiftA = _context.ShisCoderefs.SingleOrDefault(sh => sh.RefCodetype == "Shift" && sh.RefCode == "Shift A").RefDes;
                    var ShiftB = _context.ShisCoderefs.SingleOrDefault(sh => sh.RefCodetype == "Shift" && sh.RefCode == "Shift B").RefDes;
                    var ShiftC = _context.ShisCoderefs.SingleOrDefault(sh => sh.RefCodetype == "Shift" && sh.RefCode == "Shift C").RefDes;
                    ...
                    clinicList = (from c in _context.ClinicSchedules.Where(c => c.ScheWeek.Equals(date.DayOfWeek.ToString("d")) && (c.shift == shift || c.shift == "All Time") &&
                                  ...
                                  join d in _context.ShisDepartments.Where(c => c.DptCategory == reserveType)
        ...
        public List<ClinicScheduleItem> GetScheduleDataForHisOrder(string[] SoucreType, DateOnly ClinicDate, string? DoctorCode)
        {
            ...
            if (ClinicDate >= DateOnly.FromDateTime(DateTime.Today))
            {
                ...
                var Data = clinicData.Select(c => new { c.ScheWeek, c.ScheDptCode, c.ScheNoon, c.ScheDoctor, c.ScheRoom }).Distinct().OrderBy(c => c.ScheRoom).ToList();//2023.04.19 毓凱 協助修改 茲因 SHISContext 移除 OnConfiguring 統一走加密連線字串
                foreach (var clinic in Data)
                {
                    //SHISContext _inContext = new SHISContext();//2023.04.19 毓凱 協助修改 茲因 SHISContext 移除 OnConfiguring 統一走加密連線字串
                    var dept = _context.ShisDepartments.Where(c => c.DptCode == clinic.ScheDptCode).FirstOrDefault();//2023.04.19 毓凱 協助修改 茲因 SHISContext 移除 OnConfiguring 統一走加密連線字串
                    ...
                    //_inContext.Dispose();//2023.04.19 毓凱 協助修改 茲因 SHISContext 移除 OnConfiguring 統一走加密連線字串
                }
            }
            else
            {
                ...
                var Data = regData.Select(c => new { c.RegDate, c.RegDepartment, c.RegNoon, c.RegDoctorId, c.RegRoomNo }).Distinct().OrderBy(c => c.RegRoomNo).ToList();//2023.04.19 毓凱 協助修改 茲因 SHISContext 移除 OnConfiguring 統一走加密連線字串
                foreach (var reg in Data)
                {
                    //SHISContext _inContext = new SHISContext();//2023.04.19 毓凱 協助修改 茲因 SHISContext 移除 OnConfiguring 統一走加密連線字串
                    var dept = _context.ShisDepartments.Where(c => c.DptCode == reg.RegDepartment).FirstOrDefault();//2023.04.19 毓凱 協助修改 茲因 SHISContext 移除 OnConfiguring 統一走加密連線字串
