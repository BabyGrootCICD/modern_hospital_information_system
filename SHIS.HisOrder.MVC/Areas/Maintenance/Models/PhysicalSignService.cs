...
namespace SHIS.HisOrder.MVC.Areas.Maintenance.Models
{
    public class PhysicalSignService : IDisposable
    {
        ...
        public PhysicalSignService(SHISContext context)
        {
            _context = context;
        }
        ...
        public List<PhysicalSignItem> GetPhysicalSignItems(string reserveType)
        {
            ...
            List<ShisCoderef> phyCoderefs = new List<ShisCoderef>();
            List<ShisCondition> phyConditions = new List<ShisCondition>();

            #endregion
            try
            {
                phyCoderefs = _context.ShisCoderefs.Where(c => c.RefCodetype == reserveType + "_Physical_Item" &&
        ...
        public TriageReturnClass CaculateTriage(List<PhysicalConditionItem> objConditionDto)
        {
            ...
            using (CommonService cService = new CommonService(_context))
            {
                ...
                {
                    List<ShisCondition> shisConditions = new List<ShisCondition>();

                    shisConditions = _context.ShisConditions.Where(c => c.CndCodetype == condition.CODETYPE &&
                                                                      c.CndEnable == 'Y').ToList();

                    foreach (var item in shisConditions)
                    {
                        bool result1 = false;
                        bool result2 = false;
        ...
        private List<ShisCondition> GetPhysicalConditions(string PhysicalType)
        {
            List<ShisCondition> conditionLists = new List<ShisCondition>();

            try
            {
                conditionLists = _context.ShisConditions.Where(c => c.CndCodetype == PhysicalType &&
