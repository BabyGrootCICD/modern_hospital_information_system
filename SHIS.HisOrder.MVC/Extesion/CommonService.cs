...
namespace SHIS.HisOrder.MVC.Extesion
{
    public class CommonService:IDisposable
    {
        ...
        public CommonService(SHISContext context)
        {
            _context = context;
        }
        ...
        public List<ShisCoderef> GetCodeRef(string CodeType)
        {
            #region Variable Setting
            
            List<ShisCoderef> coderefs = new List<ShisCoderef>();
            
            #endregion
            try
            {
                coderefs = _context.ShisCoderefs.Where(c => c.RefCodetype == CodeType).ToList();
            }
            catch(Exception ex)
            {
        ...
        public string GetEmpName(string UserID)
        {
            ...
            string UserName = "";
            List<ShisUser> shisUsers = new List<ShisUser>();
            #endregion

            try
            {
                shisUsers = _context.ShisUsers.Where(c => c.UserIdno == UserID).ToList();
                if (shisUsers.Any())
                {
                    UserName = shisUsers.FirstOrDefault().UserNameFirstname + " " +
                               shisUsers.FirstOrDefault().UserNameMidname + " " +
                               shisUsers.FirstOrDefault().UserNameLastname;
                }
            }
            catch (Exception ex)
            {
        ...
        public List<EmployeeList> GetEmpList(string[] Type = null)
        {
            ...
            try
            {
                shisUsers = _context.ShisUsers.ToList();
        ...
        public string GetDepartmentName(string DptCode)
        {
            ...
            string DptName = "";
            List<ShisDepartment> ShisDepartments = new List<ShisDepartment>();
            #endregion

            try
            {
                ShisDepartments = _context.ShisDepartments.Where(c => c.DptCode == DptCode).ToList();
                if (ShisDepartments.Any())
                {
                    DptName = ShisDepartments.FirstOrDefault().DptName;
                }
            }
            catch (Exception ex)
            {
        ...
        public List<DepartmentList> GetDptList(string Category)
        {
            ...
            try
            {
                ...
                shisDepartments = _context.ShisDepartments.Where(c => c.DptDepth == 2 && c.DptStatus == "Y" &&
                                 c.DptCategory == (string.IsNullOrWhiteSpace(Category) ? c.DptCategory : Category)).ToList();
 
                foreach (var department in shisDepartments)
                {
