...
-- SEQUENCE: public.kmu_upload_log_logid_seq

-- DROP SEQUENCE IF EXISTS public.kmu_upload_log_logid_seq;

CREATE SEQUENCE IF NOT EXISTS public.kmu_upload_log_logid_seq
    INCREMENT 1
    START 1
    MINVALUE 1
...
ALTER SEQUENCE public.kmu_upload_log_logid_seq
    OWNER TO hisuser;

-- 2023.12.12 update by 1050325 
-- FUNCTION: public.tr_kmu_upload_log()

-- DROP FUNCTION IF EXISTS public.tr_kmu_upload_log();

CREATE OR REPLACE FUNCTION public.tr_kmu_upload_log()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
...
ALTER FUNCTION public.tr_kmu_upload_log()
    OWNER TO hisuser;


-- Table: public.kmu_upload_log

-- DROP TABLE IF EXISTS public.kmu_upload_log;

CREATE TABLE IF NOT EXISTS public.kmu_upload_log
(
    ...
    CONSTRAINT kmu_upload_log_pkey PRIMARY KEY (logid)
)

TABLESPACE pg_default;

ALTER TABLE IF EXISTS public.kmu_upload_log
    OWNER to hisuser;
-- Index: idx_inhospid
...
CREATE INDEX IF NOT EXISTS idx_inhospid
    ON public.kmu_upload_log USING btree
...
-- Trigger: tr_kmu_upload_log

-- DROP TRIGGER IF EXISTS tr_kmu_upload_log ON public.kmu_upload_log;

CREATE TRIGGER tr_kmu_upload_log
    BEFORE INSERT
    ON public.kmu_upload_log
    FOR EACH ROW
    EXECUTE FUNCTION public.tr_kmu_upload_log();
...
ALTER TABLE public."kmu_chart_MergeHistory" 
...
--病人基本資訊每日上傳--KMU_CHART daily upload--
INSERT INTO public.kmu_coderef(
	ref_codetype, ref_code, ref_name, ref_des, ref_id, ref_casetype, ref_showseq, ref_des2, modify_id, modify_time, ref_default_flag)
	VALUES ('MOHD_TK','TK_02','https://localhost:7287/api/UploadTask/MOHD_TK_02','KMU_CHART daily upload','00000000000000234','Y',1,',
https://localhost:7287/api/UploadTask/MOHD_TK_02','1050325','2023-11-16',' ');
--病歷合併每日上傳--Merged or Undo KMU_CHART daily upload--
INSERT INTO public.kmu_coderef(
	ref_codetype, ref_code, ref_name, ref_des, ref_id, ref_casetype, ref_showseq, ref_des2, modify_id, modify_time, ref_default_flag)
	VALUES ('MOHD_TK','TK_03','https://localhost:7287/api/UploadTask/MOHD_TK_03','Merged or Undo KMU_CHART daily upload','00000000000000233','Y',1,',
https://localhost:7287/api/UploadTask/MOHD_TK_03','1050325','2023-11-16',' ');

--## 注意: gateway帳號密碼是否與MOHD建立一致.(以HGH做範例，各院區依照需求自行建立)

INSERT INTO public.kmu_users(
