...
namespace SHIS.Lab.WebAPI.Service
{
    public class TransferDataService
    {
        ...
        public ResultDTO GetPatientById(string? patientId)
        {
            ...
            try
            {
                ...
                {
                    var patientExist = _context.ShisChart.SingleOrDefault(p => p.HealthId == patientId);
                    ...
                    {
                        var patients = _context.ShisChart.Where(p => p.HealthId == patientId).ToList();
                        result.Shischart = new List<ShisChart> { };
                        if(patients != null && patients.Count > 0)
                        {
                        foreach(var pt in patients)
                           {
                               result.Shischart.Add(pt);
                           }
