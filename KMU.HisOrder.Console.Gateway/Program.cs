...
using KMU.HisOrder.Console.Gateway.Models;
...
using System.IO;
using KMU.HisOrder.Console.Gateway.Models_MOHD;
...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        try
        {
            ...
            {
                //院區
                var hospdata = context.KmuCoderefs.Where(c => c.RefCodetype == "HospitalCode").FirstOrDefault();
                ...
                if (duplicateInhospids.Any())
                {
                    duplicateInhospids.ForEach(c =>
                    {
                        using (var context_du = new GatewayContext())
                        {
                            ...
                            KmuUploadLog uploadlog = new KmuUploadLog();
                            uploadlog.Logid = -1;
                            ...
                            context_du.KmuUploadLogs.Add(uploadlog);
                ...
                if (uploadCase.Any())
                {
                    ...
                    foreach (var item in uploadCase)
                    {
                        using (var context_sub = new GatewayContext())
                        {
                            ...
                            #region 必要資訊
                            var pInfo = context_sub.KmuCharts.Where(c => c.ChrHealthId == item.RegHealthId).FirstOrDefault();
                            if (pInfo == null)
                            {
                                using (var _errorContext = new GatewayContext())
                                {
                                    logger.Fatal($"kmu_chart No patient found: {item.RegHealthId}");
                                    KmuUploadLog _uploadlog = new KmuUploadLog();
                                    _uploadlog.Logid = -1;
                                    ...
                                    _uploadlog.ResultMessage = "kmu_chart No patient found: " + item.RegHealthId;
                                    _uploadlog.ResultStatusCode = "999";
                                    _uploadlog.ResultStatusDesc = "Exception";

                                    _errorContext.KmuUploadLogs.Add(_uploadlog);
                            ...
                            var regdptparent = context_sub.KmuDepartments.Where(d => d.DptCode == rInfo.RegDepartment).Select(d => d.DptParent).SingleOrDefault();
                            root.RegInfo = new RegInfo()
                            {
                                HospCode = hosp_code,
                                RegDate = rInfo.RegDate,
                                DeptCode = rInfo.RegDepartment,
                                DeptName = context_sub.KmuDepartments.Where(d => d.DptCode == rInfo.RegDepartment).Select(e => e.DptName).FirstOrDefault(),
                                Noon = rInfo.RegNoon,
                                ...
                                DoctorName = context_sub.KmuUsers.Where(c => c.UserIdno == rInfo.RegDoctorId).Select(d => d.UserNameFirstname + d.UserNameMidname + d.UserNameLastname).FirstOrDefault(),
                                ...
                                dept_parent = context_sub.KmuDepartments.Where(d => d.DptCode == rInfo.RegDepartment).Select(d => d.DptParent).SingleOrDefault(),
                                dept_parent_name = context.KmuDepartments.Where(d => d.DptCode == regdptparent).Select(r => r.DptName).SingleOrDefault(),
                            ...
                            if (_inConnSatus == ConnectionStatus.Online)
                            {
                                ...
                                string apiUrl = context.KmuCoderefs
                                ...
                                try
                                {
                                    ...
                                    context_sub.KmuUploadLogs.Add(uploadlog);
    ...
    static async Task<string> upload_merge_patient(ConnectionStatus inConnSatus = ConnectionStatus.Online)
    {
        try
        {
            ...
            using (var context = new GatewayContext())
            {
                ...
                //step1 搜尋需要上傳的Case
                var uploadCase = context.
                    KmuChartMergeHistories.Where(d => d.UploadStatus == "N").ToList();
                    ...
                    foreach (var item in uploadCase)
                    {
                        using (var context_sub = new GatewayContext())
                        {
                            ...
                            if (item == null)
                            {
                                using (var _errorContext = new GatewayContext())
                                {
                                    logger.Fatal($"kmu_chart No patient found: {item.ChrHalthId}");
                                    KmuUploadLog _uploadlog = new KmuUploadLog();
                                    _uploadlog.Logid = -1;
                                    _uploadlog.RegDate = null;
                                    _uploadlog.Option = userChoice != null ? Convert.ToChar(userChoice) : ' ';
                                    ...
                                    _uploadlog.TargetUrl = null;
                                    _uploadlog.ResultSuccess = false;
                                    _uploadlog.ResultMessage = "kmu_chart No patient found: " + item.ChrHalthId;
                                    _uploadlog.ResultStatusCode = "999";
                                    _uploadlog.ResultStatusDesc = "Exception";
                                ...
                                try
                                {
                                    ...
                                    if (response.IsSuccessStatusCode)
                                    {
                                        ...
                                        if (result.isSuccess == true)
                                        {
                                            ...
                                            //update KMU_MergeHistory
                                            item.UploadStatus = "Y";
                                            //item.upload_time = DateTime.Now;
                                            context_sub.KmuChartMergeHistories.Update(item);
                                        }
                                        else
                                        {
                                            //logger.Error(responseContent);
                    ...
                    foreach (var item in uploadCase)
                    {
                        using (var context_sub = new GatewayContext())
                        {
                            ...
                            if (item == null)
                            {
                                using (var _errorContext = new GatewayContext())
                                {
                                    logger.Fatal($"kmu_chart No patient found: {item.ChrHealthId}");
                                    KmuUploadLog _uploadlog = new KmuUploadLog();
                                    _uploadlog.Logid = -1;
                                    _uploadlog.RegDate = null;
                                    _uploadlog.Option = userChoice != null ? Convert.ToChar(userChoice) : ' ';
                                    ...
                                    _uploadlog.TargetUrl = null;
                                    _uploadlog.ResultSuccess = false;
                                    _uploadlog.ResultMessage = "kmu_chart No patient found: " + item.ChrHealthId;
                                    _uploadlog.ResultStatusCode = "999";
                                    _uploadlog.ResultStatusDesc = "Exception";
                                ...
                                try
                                {
                                    ...
                                    if (response.IsSuccessStatusCode)
                                    {
                                        ...
                                        if (result.isSuccess == true)
                                        {
                                            ...
                                            context_sub.KmuCharts.Update(item);
                                        }
                                        else
                                        {
                                            //logger.Error(responseContent);
        ...
        try
        {
            using (var context = new GatewayContext())
            {
                ...
                if (exportResult.isSuccess == true)
                {
                    ...
                    {
                        var pInfo = context.KmuCharts.Where(c => c.ChrHealthId == root.PatientInfo.HealthId).FirstOrDefault();
                        ...
                        rInfo.UploadStatus = "Y";
                        rInfo.UploadTime = DateTime.Now;
                        context.KmuCharts.Update(pInfo);
                    ...
                    {
                        var pInfo = context.KmuCharts.Where(c => c.ChrHealthId == patientinfo.HealthId).FirstOrDefault();
                        //update registration、kmuchart
                        pInfo.UploadStatus = "Y";
                        pInfo.UploadTime = DateTime.Now;
                        context.KmuCharts.Update(pInfo);
                    ...
                    {
                        var mInfo = context.KmuChartMergeHistories.Where(c => c.MhHealthId == mergePatient.MhHealthId).FirstOrDefault();
                        //update registration、kmuchart
                        mInfo.UploadStatus = "Y";
                        mInfo.UploadTime = DateTime.Now;
                        context.KmuChartMergeHistories.Update(mInfo);
    ...
    static async Task<ResultDTO> GetNewJwtToken()
    {
        ...
        if (hosp_code != "unknown")
        {
            using (var context = new GatewayContext())
            {
                apiUrl = context.KmuCoderefs
                               .Where(c => c.RefCodetype == "MOHD_TK" && c.RefCode == "JWT").Select(c => c.RefName).FirstOrDefault();
                mohdData = context.KmuUsers.Where(c => c.Creator == "MOHDGW").FirstOrDefault();
        ...
        try
        {
            using (var context_sub = new GatewayContext())
            {
                ...
                root.RegInfo = new RegInfo()
                {
                    ...
                    FollowDesc = rInfo.RegFollowDesc,
                    dept_parent = context_sub.KmuDepartments.Where(d => d.DptCode == rInfo.RegDepartment).Select(d => d.DptParent).SingleOrDefault(),
                    dept_parent_name = context_sub.KmuDepartments.Where(d => d.DptCode == regdptparent).Select(r => r.DptName).SingleOrDefault(),
